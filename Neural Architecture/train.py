...
def sort_data(data, tags):
	...
	data = map(data.__getitem__, indexes)
	tags = map(tags.__getitem__, indexes)
	return data, tags

def prepare_data(seqs, vocab):
	clipped_seqs = [seq[:CLIP] if len(seq) > CLIP else seq for seq in seqs]
	vectorized_seqs = [[vocab[tok] if tok in vocab else 1 for tok in seq] for seq in clipped_seqs]
...
def train(train_data, train_tags):
	...
	for epoch in range(EPOCHS):
		...
		for i in range(0, size): 
			train_data, train_tags = shuffle_data(train_data, train_tags)
			if i % 500 == 0:
				print("Iteration = {}".format(i))
				sys.stdout.flush()
			model.zero_grad()
			model.hidden = model.init_hidden()
			sentences_in = train_data[i]
			targets = train_tags[i]
			tag_scores = model(sentences_in.t())
			loss = loss_function(tag_scores, targets)
			loss.backward()
			optimizer.step()
		acc = test() # Check Validation Accuracy at each train epoch
		if best_saved_score < acc:
			torch.save(model, MODE + "_" + str(epoch) + ".txt")
			best_saved_score = acc
	return model
...
text, train_tags, summary = get_data(trainfile)
dev_text, val_tags, dev_summary = get_data(testfile)
print("Total Read Time = {}".format(time.time() - read_start))

train_data = [s*SUMMARY_WEIGHT + t for t, s in zip(text, summary)]
...
(train_data, train_tags) = sort_data(train_data, train_tags)
(val_data, val_tags) = sort_data(val_data, val_tags)
print("Total Sort Time = {}".format(time.time() - sort_start))

train_data = prepare_data(train_data, word_to_ix)
val_data = prepare_data(val_data, word_to_ix)
train_tags = prepare_tags(train_tags, tag_to_ix)
val_tags = prepare_tags(val_tags, tag_to_ix)

# Define model and Learning parameters
if torch.cuda.is_available():
	model = Sentiment(MODE, EMBEDDING_DIM, HIDDEN_DIM, len(word_to_ix)+2, len(tag_to_ix), word_to_ix).cuda()
else:
	model = Sentiment(MODE, EMBEDDING_DIM, HIDDEN_DIM, len(word_to_ix)+2, len(tag_to_ix), word_to_ix)

# Train the model
train_start = time.time()
train(train_data, train_tags)
